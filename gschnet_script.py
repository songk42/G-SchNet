...
import gschnet_trainer
import generation

from nn_classes import (
    AtomwiseWithProcessing,
...
dataset_name_to_class_mapping = {"qm9": QM9gen, "template_data": TemplateData}
...
def get_parser():
    ...
    cmd_parser.add_argument(
        ...
        "(specify with environment variable" + " CUDA_VISIBLE_DEVICES)",
        action="store_true",
    )
    cmd_parser.add_argument(
        "--batch_size",
    ...
    train_parser.add_argument(
        "datapath", help="Path / destination of dataset " "directory"
    )
    ...
    train_parser.add_argument(
        "--remove_invalid",
        type=bool,
        help="Remove invalid molecules from dataset"
        "(default: %(default)s)",
        default=True,
    )
    ...
    gen_parser.add_argument(
        "amount_gen", type=int, help="The amount of generated molecules"
    )
    gen_parser.add_argument(
        "--show_gen",
        help="Whether to open plots of generated " "molecules for visual evaluation",
        action="store_true",
    )
    gen_parser.add_argument(
        "--chunk_size",
        type=int,
        help="The size of mini batches during generation " "(default: %(default)s)",
        default=1000,
    )
    gen_parser.add_argument(
        "--max_length",
        type=int,
        help="The maximum number of atoms per molecule " "(default: %(default)s)",
        default=35,
    )
    gen_parser.add_argument(
        "--file_name",
    ...
    gen_parser.add_argument(
        "--temperature",
        type=float,
        help="The temperature T to use for sampling " "(default: %(default)s)",
        default=0.1,
    )
    ...
    #######  G-SchNet  #######
    gschnet_parser = argparse.ArgumentParser(add_help=False, parents=[model_parser])
    gschnet_parser.add_argument(
        "--features",
        type=int,
    ...
    gschnet_parser.add_argument(
        "--interactions",
        type=int,
        help="Number of regular SchNet interaction " "blocks (default: %(default)s)",
        default=9,
    )
    gschnet_parser.add_argument(
        "--cutoff",
    ...
    train_subparsers.add_parser(
        "gschnet", help="G-SchNet help", parents=[train_parser, gschnet_parser]
    )
    ...
    eval_subparsers.add_parser(
        "gschnet", help="G-SchNet help", parents=[eval_parser, gschnet_parser]
    )

    gen_subparsers = subparser_gen.add_subparsers(
        dest="model", help="Model-specific " "arguments"
    )
    gen_subparsers.required = True
    gen_subparsers.add_parser(
        "gschnet", help="G-SchNet help", parents=[gen_parser, gschnet_parser]
    )
...
def get_model(args, parallelize=False):
    ...
    out_module_type = AtomwiseWithProcessing(
        n_in=args.features,
        n_out=1,
        n_layers=5,
    ...
    preprocess_dist = EmbeddingMultiplication(
        representation.embedding,
        in_key_types="_next_types",
        in_key_representation="representation",
        out_key="preprocessed_representation",
    )
    out_module_dist = AtomwiseWithProcessing(
        n_in=args.features,
    ...
    model = spk.atomistic.AtomisticModel(
        representation, [out_module_type, out_module_dist]
    )
...
def train(args, model, train_loader, val_loader, device):
    ...
    metrics = [
        KLDivergence(
            target="_type_labels", model_output="type_predictions", name="KLD_types"
        ),
        KLDivergence(
            target="_labels",
    ...
    # initialize trainer
    trainer = gschnet_trainer.Trainer(
        args.modelpath,
        model,
        loss,
        optimizer,
    ...
    generation.main(gschnet_trainer.GenerationArgs(-1, 10000, 500))
...
def evaluate(args, model, train_loader, val_loader, test_loader, device):
    ...
    metrics = [
        KLDivergence(
            target="_labels", model_output="distance_predictions", mask="_dist_mask"
        ),
        KLDivergence(target="_type_labels", model_output="type_predictions"),
    ]
...
def generate(args, train_args, model, device):
    ...
    if args.print_file:
        progress = (
            lambda x, y: print(f"Generated {x}.", flush=True)
            if x >= y
    ...
    if args.show_gen:
        ...
        asv.view(ats)
        print(
            f"Total number of atoms placed: {n_total_atoms} "
            f"(avg {n_total_atoms / n_molecules:.2f})",
            flush=True,
        )
...
def main(args):
    ...
    if args.mode == "train":
        ...
        if args.pretrained_path is not None:
            ...
            if (
                os.path.exists(model_chkpt_path)
                ...
                if not os.path.exists(pretrained_chkpt_path):
                    ...
                    chkpt_files = [
                        f
                        for f in os.listdir(pretrained_chkpt_path)
                    ...
                    if len(chkpt_files) == 0:
                        ...
                        epoch = max(
                            [int(f.split(".")[0].split("-")[-1]) for f in chkpt_files]
                        )
                        chkpt = os.path.join(
                        ...
                        copyfile(
                            chkpt,
                            os.path.join(
                                model_chkpt_path, f"checkpoint-{epoch}.pth.tar"
                            ),
                        )
    ...
    if args.mode in ["train", "eval"]:
        ...
        if train_args.dataset_name == "qm9":
            data_args["remove_invalid"] = args.remove_invalid
        data = dataclass(args.datapath, **data_args)
        ...
        collate = lambda x: collate_atoms(
            x,
